...
function transformDictionary(dictionary) {
  ...
  for (const entr of dictionary.entries()) {
    entries.push(entr);
  }
...
function checkPresence(dictionary, keys) {
  const resultArr = [];
  keys.forEach((key) => resultArr.push(dictionary.has(key)));
  return resultArr;
...
function getFilteredDictionarySize(dictionary, filter) {
  ...
  for (const [key, value] of dictionary.entries()) {
    if (filter(key, value)) {
      filterDictionary.set(key, value);
    }
  }
  return filterDictionary.size;
...
function sortByValues(dictionary) {
  const entriesArray = [...dictionary.entries()];
  entriesArray.sort((a, b) => b[1] - a[1]);
  const sortedDictionary = new Map(entriesArray);
...
function convertSetToDictionary(set) {
  ...
  for (const element of set) {
    const firstCharCode = element.charCodeAt(0);
    dictionary.set(element, firstCharCode);
  }
  return dictionary;
